...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    # Auto-close the JARVIS HUD map when an unrelated tool is invoked,
    # so the assistant returns to its initial state on any command that
    # is not the map itself.
    if tool not in ("jarvis_map", "home_screen"):
        try:
            from actions.jarvis_map import close_jarvis_map
            if close_jarvis_map():
                print("[Executor] 🗺️ Closed JARVIS map (unrelated tool).")
        except Exception:
            pass

    if tool == "home_screen":
        from actions.jarvis_map import close_jarvis_map, is_jarvis_map_open
        was_open = is_jarvis_map_open()
        close_jarvis_map()
        return "Returning to home screen, sir." if was_open else "Already on home screen, sir."
    ...
    elif tool == "check_messages":
        from actions.check_messages import check_messages
        return check_messages(parameters=parameters, player=None) or "Done."

    elif tool == "read_last_from":
        from actions.check_messages import read_last_from
        return read_last_from(parameters=parameters, player=None) or "Done."

    elif tool == "reply_message":
        from actions.send_message import reply_message
        return reply_message(parameters=parameters, player=None) or "Done."
