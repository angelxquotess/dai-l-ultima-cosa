...
send_message
  receiver: string (required) — nome del contatto WhatsApp
  message_text: string (required) — testo del messaggio da inviare
  platform: string (required) — deve essere "whatsapp" (unico canale supportato)

check_messages
  (nessun parametro) — risponde a "ho messaggi non letti?" leggendo la coda del wa-bridge

read_last_from
  contact: string (required) — nome del contatto WhatsApp di cui leggere l'ultimo messaggio ricevuto

reply_message
  message_text: string (required) — testo con cui rispondere all'ultimo messaggio ricevuto
...
Goal: "Ho messaggi non letti?"
Steps:

check_messages |

Goal: "Dimmi cosa mi ha detto Anna"
Steps:

read_last_from | contact: Anna

Goal: "Rispondi al messaggio dicendo 'arrivo tra 5 minuti'"
Steps:

reply_message | message_text: "arrivo tra 5 minuti"
